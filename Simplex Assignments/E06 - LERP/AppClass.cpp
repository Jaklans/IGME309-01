...
void Application::Display(void)
{
	...
	vector3 startPoint = m_stopsList[(index) % (int)m_stopsList.size()];
	vector3 endPoint = m_stopsList[(index + 1) % (int)m_stopsList.size()];

	//calculate the current position
	vector3 v3CurrentPos = glm::lerp(startPoint, endPoint, fTimer / 2.0f);
